...
const Products = () => {
  ...
  const {query,results,products}=useSelector(state=>state.search)
  ...
  const shuffledList = useMemo(() => {
    let shoesToDisplay = query ? results : products;
    if (shoesToDisplay && shoesToDisplay.length) {
      return shuffleArray(shoesToDisplay);
    }
    return [];
  }, [query, results, products, Filter, shuffleArray]);

  useEffect(() => {
    setPageIndex(1);
  }, [query, results, products, Filter])
     ...
     useEffect(() => {
      ...
      const currentShoes=useMemo(()=>shuffledList.slice(startPageIndex,endPageIndex),[shuffledList, startPageIndex, endPageIndex])
    ...
    <div className="w-full flexCol "  ref={pageRef}>
      <div className="w-full grid  grid-cols-10  bg-primary h-10 md:h-12 items-center  md:px-10 divide-x-[1px] border-b-2  ">
        ...
        <h1 className=" col-span-4 sm:col-span-6 text-sm sm:text-base  font-medium text-black/80 h-full flexCenter">
          {query?results.length:products.length} Results
        </h1>
